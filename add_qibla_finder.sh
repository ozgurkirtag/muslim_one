...
flutter pub add geolocator flutter_compass

mkdir -p lib/screens/qibla

cat > lib/screens/qibla/qibla_screen.dart <<'DART'
import 'dart:async';
...
class _QiblaScreenState extends State<QiblaScreen> {
  ...
  static const _texts = <String, Map<String, String>>{
    ...
    'turn': {
      ...
      'fr': 'Tenez le téléphone à plat et alignez la flèche dorée avec la Kaaba.',
      'de': 'Halte das Telefon flach und richte den goldenen Pfeil auf die Kaaba aus.',
      ...
      'ms': 'Pegang telefon rata dan sejajarkan anak panah emas dengan ikon Kaabah.',
      'ur': 'فون کو سیدھا رکھیں اور سنہری تیر کو کعبہ کے نشان سے ملائیں۔',
      'ru': 'Держите телефон ровно и совместите золотую стрелку со значком Каабы.',
    },
    'aligned': {
    ...
    'calibrate': {
      'tr': 'Pusula sapıyorsa telefonu birkaç kez sekiz şeklinde hareket ettirin.',
      ...
      'ru': 'Если компас отклоняется, двигайте телефон по траектории восьмёрки.',
    },
    'locationOff': {
      ...
      'fr': 'La localisation est désactivée. Activez-la pour calculer la Qibla.',
      ...
      'ms': 'Perkhidmatan lokasi dimatikan. Aktifkan lokasi untuk mengira Kiblat.',
    ...
    'permission': {
      ...
      'de': 'Für die Qibla-Berechnung ist die Standortberechtigung erforderlich.',
    ...
    'noSensor': {
      'tr': 'Bu cihazda pusula sensörü algılanmadı. Kıble açısını aşağıdan görebilirsiniz.',
      'en': 'No compass sensor was detected. You can still use the Qibla bearing below.',
    ...
    'privacy': {
      'tr': 'Konum yalnızca kıble yönünü hesaplamak için cihazınızda kullanılır.',
      'en': 'Your location is used on-device only to calculate the Qibla direction.',
      'ar': 'يُستخدم موقعك على الجهاز فقط لحساب اتجاه القبلة.',
      'fr': 'Votre position est utilisée sur l’appareil uniquement pour calculer la Qibla.',
      'de': 'Dein Standort wird nur auf dem Gerät zur Qibla-Berechnung verwendet.',
      'es': 'Tu ubicación se usa solo en el dispositivo para calcular la Qibla.',
      ...
      'ur': 'آپ کا مقام صرف قبلہ کی سمت معلوم کرنے کے لیے فون پر استعمال ہوتا ہے۔',
  ...
  bool get _isAligned {
    ...
    final difference =
        ((_qiblaBearing! - _heading! + 540) % 360) - 180;
            ...
            : _locationServiceDisabled
                ...
                : _position == null
                    ? _PermissionView(
                        ...
                        button: _permissionDeniedForever
                            ? _t('settings')
                            : _t('retry'),
...
if "android.permission.ACCESS_FINE_LOCATION" not in text:
    ...
    text = text[: first_close + 1] + "\n" + permission_block + text[first_close + 1 :]

manifest.write_text(text)

# iOS foreground location purpose string
...
if plist.exists():
    ...
    if "NSLocationWhenInUseUsageDescription" not in text:
        entry = (
            ...
            "\t<string>Muslim One uses your location only to calculate the Qibla direction.</string>\n"
        )
        text = text.replace("</dict>", entry + "</dict>")
        plist.write_text(text)

# Route constant
routes = Path("lib/core/routes/app_routes.dart")
text = routes.read_text()

if "qibla" not in text.lower():
    marker = "  static const String prayerTimes = '/prayer-times';"
    if marker not in text:
        marker = "  static const String settings = '/settings';"
    if marker not in text:
...
# Main import and route
main = Path("lib/main.dart")
text = main.read_text()

if "qibla_screen.dart" not in text:
    candidates = [
    ...
    for marker in candidates:
        ...
        raise SystemExit("Main import insertion point not found.")

if "AppRoutes.qibla" not in text:
    candidates = [
    ...
    for marker in candidates:
        ...
        raise SystemExit("Main route insertion point not found.")
...
if "pushNamed(AppRoutes.qibla)" not in text:
    patterns = [
        (
            """            case 'prayer':
...
""",
    ...
    if not changed:
        raise SystemExit("Qibla home navigation insertion point not found.")
...
git commit -m "Add worldwide Qibla finder"
git push
